...
For a regression-detection handoff:

```
Test-writer handoff for slice 5.4: REGRESSION DETECTED
- Regression check ran against prior slices' scope: <scoped command>
- Failing tests: <file:test names>
- Failure output excerpt: <key lines>
- New slice tests NOT written; orchestrator should route developer to fix the regression first, then re-dispatch test-writer
```

For a normal handoff:

```
Test-writer handoff for slice 5.3:
- Regression check: green (or skipped — slice 1)
- ACs covered: 5.3.1, 5.3.2, 5.3.3, 5.3.4, 5.3.5, 5.3.6 (6 ACs)
- Tests added: 4 new (extending apps/api/src/dispatch/dispatch.service.spec.ts); 2 fresh (apps/web/meetings/dispatch-popup.spec.tsx)
- Test levels: 5 unit, 1 integration (5.3.3 needs real Supabase JWT validation)
- Fixtures used: tests/fixtures/users.ts (three-user RLS); tests/fixtures/recall.ts (mock Recall.ai responses)
- Compliance assertions added: 5.3.3 (OWASP A07 — JWT validation), 5.3.5 (OWASP A01 — owner_id server-set)
- Notes: 5.3.2 input validation extended existing url-validator.spec.ts rather than fresh
```
